...
//      bug behind the "Relax+ Cash Back" incident, where classifyAndRespond's safety net only
...
//     own context resolution and classifyAndRespond's safety net (simulated with the API
...
function buildModule(dataJsonText) {
  ...
  const full = stub + logicSrc + '\nconst window = {};\nmodule.exports = { classifyKeyword, classifyAndRespond, CARDS, GENERAL_SCRIPTS, OOS, SCRIPT_INDEX, slug };\n';
...
async function main() {
  ...
  const { classifyKeyword, classifyAndRespond, CARDS, OOS, slug } = mod;
  ...
  for (const card of CARDS) {
    ...
    for (const topic of card.topics) {
      ...
      for (const kw of topic.keywords) {
        ...
        // Check 3: same context-only follow-up through classifyAndRespond's safety net,
        // simulating the model being unavailable — the exact path that missed the
        // Relax+ Cash Back fix before it was patched.
        checks++;
        const llmResult = await classifyAndRespond(kw, history);
        if (!hasSegment(llmResult.script && llmResult.script.label, card, topic)) {
          failures.push({
            check: 'context-followup (classifyAndRespond safety net)',
            card: card.id, topic: topic.topic, keyword: kw, message: kw, history: history[0].content,
  ...
  // by classifyAndRespond's context-card safety net. Caught live: "I lost my card"
  ...
  for (const card of CARDS) {
    ...
    for (const oos of OOS) {
      for (const kw of oos.keywords) {
        ...
        if (llmResult.script) {
          failures.push({
            check: 'oos-after-context (classifyAndRespond safety net)',
